...
export default function VendorDetailScreen({ route, navigation }: any) {
  ...
  return (
    <SafeScreen style={styles.container}>
      <AdminHeader title="Vendor" navigation={navigation} showBack={true} />

      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        {/* Vendor Header Card */}
        <View style={styles.card}>
          <Text style={styles.vendorName} numberOfLines={3}>
            {vendor.name}
          </Text>
          <View style={styles.badgesRow}>
            ...
            <View style={[styles.statusBadge, getStatusBadgeStyle(vendor.status)]}>
              <Text style={[styles.badgeText, getStatusTextStyle(vendor.status)]}>
                {`${String(vendor.status || 'unknown').charAt(0).toUpperCase()}${String(vendor.status || 'unknown').slice(1)}`}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.card}>
          {vendor.status === 'pending' && (
            <>
              <TouchableOpacity style={[styles.actionButton, styles.approveBtn]} onPress={handleApprove} activeOpacity={0.85}>
          ...
          {vendor.status === 'active' && (
            <TouchableOpacity style={[styles.actionButton, styles.suspendBtn]} onPress={handleSuspend} activeOpacity={0.85}>
              <Text style={styles.actionButtonText}>Suspend Vendor</Text>
            </TouchableOpacity>
          )}

          {vendor.status === 'suspended' && (
            <TouchableOpacity style={[styles.actionButton, styles.reactivateBtn]} onPress={handleReactivate} activeOpacity={0.85}>
        ...
        {showRejectInput && (
          <View style={styles.card}>
            ...
            <TextInput
              ...
              numberOfLines={4}
            />
            <TouchableOpacity style={[styles.actionButton, styles.confirmRejectBtn]} onPress={handleReject} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  scrollContent: { paddingHorizontal: 16, paddingVertical: 20, paddingBottom: 32 },
  card: {
  ...
  badgesRow: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', marginBottom: 16, gap: 8 },
  typeBadge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, marginBottom: 4 },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, marginBottom: 4 },
  ...
  infoText: { flex: 1, fontSize: 14, color: '#6B7280', marginLeft: 8, lineHeight: 20 },
